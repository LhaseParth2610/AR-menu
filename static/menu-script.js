...
function openFoodDetail(itemId) {
    ...
    try {
        ...
        if (modelViewer && item.modelPath) {
            ...
            modelViewer.setAttribute('ios-src', '/static/' + item.Iospath);
            // Apply the scale correction
            modelViewer.scale = `${item.modelScale} ${item.modelScale} ${item.modelScale}`;
            console.log('Set model path to:', '/static/' + item.modelPath);
            console.log('Set model path to:', '/static/' + item.Iospath);

            console.log('Applied model scale:', item.modelScale);
        } else if (modelViewer) {
            modelViewer.src = '';
        ...
        modelViewer.canActivateAR().then(canActivate => {
            if (canActivate) {
